...
import {
  ...
  getUsersWhoHaveUserLessonForThisCursus,
 ...
 */
export async function changeOrderLessonsController(
  req: Request,
  res: Response
): Promise<Response<ApiResponse<LessonData[] | any>>> {
  if (!req.params.id)
    throw new AppError(
      400,
 ...
 */
export async function addLessonController(
  req: Request,
  res: Response
): Promise<Response<ApiResponse<LessonData[]>>> {
...
export async function deleteLessonController(
  ...
  const { cursusId, themeId } = await getCursusIdAndThemeIdForThisLesson(
    lessonId
  );
  const usersWhoHaveUserLessonForThisCursus =
    await getUsersWhoHaveUserLessonForThisCursus(cursusId);
