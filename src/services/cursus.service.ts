...
import { AppError, extractDbErrorCode, throwDbError } from "../utils/AppError.js";
...
export async function  changeOrderCursus(cursusId: number, move: 'up' | 'down', userId: number): Promise<ApiResponse> {
  const dbErrorMessage = "changeOrderCursus function in cursus service failed";
  const dbErrorMessageFront = "Le changement d'ordre des cursus a échoué, veuillez réessayer ultérieurement ou contacter le support.";
  
  let targetCursus: Cursus | null;
  
  try {
    targetCursus = await Cursus.findOne({ where: { id: cursusId } });
    ...
    if (error instanceof AppError) throw error;
    throwDbError(500, dbErrorMessage, dbErrorMessageFront, error, 'Cursus', 'findOne');
  }
  ...
  if(move === 'up') {
    ...
    let cursusToSwap: Cursus | null;

    try {
      cursusToSwap = await Cursus.findOne({ where: { theme_id: targetCursus.theme_id, order: targetCursus.order - 1 } });
    ...
    try {
      ...
      throwDbError(500, dbErrorMessage, dbErrorMessageFront, error, 'Cursus', 'update');
    }
  } else if (move === 'down') {
    let allCursusInTheme: Cursus[];
    
    try {
      allCursusInTheme = await Cursus.findAll({ where: { theme_id: targetCursus.theme_id } });
      ...
      throwDbError(500, dbErrorMessage, dbErrorMessageFront, error, 'Cursus', 'findAll');
    }
...
export async function deleteCursus(cursusId: number): Promise<void> {
  const dbErrorMessage = "deleteCursus function in cursus service failed";
  const dbErrorMessageFront = "La suppression du cursus a échoué, veuillez réessayer ultérieurement ou contacter le support.";
  
  let cursusToDelete: Cursus | null;

  try {
    cursusToDelete = await Cursus.findByPk(cursusId);
  } catch (error) {
    throwDbError(500, dbErrorMessage, dbErrorMessageFront, error, 'Cursus', 'findByPk');
  }
  ...
  try {
    ...
    throwDbError(500, dbErrorMessage, dbErrorMessageFront, error, 'Cursus', 'destroy');
  }
}
...
export async function updateCursus(cursusId: number, newCursusName: string, newCursusPrice: number, requestorId: number): Promise<void> {
  const dbErrorMessage = "updateCursus function in cursus service failed";
  const dbErrorMessageFront = "La mise à jour du cursus a échoué, veuillez réessayer ultérieurement ou contacter le support.";
  
  let cursusToUpdate: Cursus | null;

  try {
    cursusToUpdate = await Cursus.findByPk(cursusId);
  } catch (error) {
    throwDbError(500, dbErrorMessage, dbErrorMessageFront, error, 'Cursus', 'findByPk');
  }
